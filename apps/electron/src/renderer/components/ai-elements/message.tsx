...
const MarkdownLink = React.memo(function MarkdownLink({
  ...
  // 会话路径解析上下文（候选 basePaths + 所属 sessionId），供本地文件 chip 穿透授权边界
  const pathResolutionContext = React.useContext(BasePathsContext)
  const contextBasePaths = pathResolutionContext?.basePaths
    ...
    if (isAbsoluteFilePath(filePath)) {
      return (
        <ResolvableFilePathChip
          filePath={filePath}
          basePaths={contextBasePaths}
          sessionId={pathResolutionContext?.sessionId}
          fallback={<span>{linkChildren}</span>}
        />
      )
    }
  }
...
/**
 * 将 Agent Markdown 中的本地图片路径转换为经主进程授权的可加载 URL（guru-file/file 协议）。
 *
...
const MarkdownInlineCode = React.memo(function MarkdownInlineCode({
  ...
  // 兜底：从 context 读附加 basePaths 与会话（避免穿透 SDKMessageRenderer / ContentBlock 等中间层）
  const pathResolutionContext = React.useContext(BasePathsContext)
  ...
  if (text) {
    ...
    if (merged.length > 0 && isRelativeFilePath(text)) {
      ...
      if (turnFileMap && turnFileMap.size > 0) {
        ...
        if (abs) {
          // 本轮实际成功读写过的文件具有工具调用证据，直接渲染 Chip，避免正文完成后再闪烁升级。
...
export const UserMessageContent = React.memo(
  function UserMessageContent({ children, className, ...props }: UserMessageContentProps): React.ReactElement {
    ...
    return (
      <div className={cn('user-message-bubble relative block w-fit max-w-full whitespace-normal [overflow-wrap:anywhere] rounded-[12px] bg-secondary/75 px-3.5 py-2.5', className)} {...props}>
        <div
          ref={contentRef}
          className={cn(
