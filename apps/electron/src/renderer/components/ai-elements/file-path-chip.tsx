...
export function resolveFilePathEntry(filePath: string, bases: string[], sessionId?: string): Promise<FileResolutionCacheEntry> {
...
export function FilePathChip({ filePath, basePath, basePaths, sessionId, className }: FilePathChipProps): React.ReactElement {
  ...
  const resolveCurrentPath = React.useCallback((): Promise<void> => {
    ...
    promise = resolveFilePathEntry(cleanPath, candidateBases, resolvedSessionId ?? undefined)
      .then((entry) => {
