/**
 * CredentialDialog — 集成目录「单凭据」条目的密钥输入弹窗
 *
 * 与本地 McpCredentialActions 内嵌的 SaveCredentialDialog 同构，但面向目录凭据：
 * header/env 名由目录条目固定（用户只需粘贴值），并带官方控制台获取入口。
 * 密钥值只经 IPC 进入 safeStorage/Keychain：不回显、不落盘、不写入 mcp.json。
 */
