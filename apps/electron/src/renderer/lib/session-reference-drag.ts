/**
 * session-reference-drag — 左侧 Agent 会话行拖拽到 Agent 输入框的引用协议
 *
 * 自定义 MIME 用于可靠识别来源，text/plain 仅作为宿主兼容兜底（写入会话 mention
 * 的 Markdown 协议 &session:<id>::<label>，供不支持自定义 MIME 的宿主还原）。
 *
 * 命名/事件前缀对齐本地 file-panel-drag 惯例（x-guru- / guru:），
 * 与上游 Proma 的 x-proma-/proma: 不同。
 */
