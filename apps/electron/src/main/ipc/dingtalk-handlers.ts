/**
 * dingtalk-handlers — 从 src/main/ipc.ts 抽出的 IPC 处理器分组
 *
 * 抽取自 ipc.ts banner「钉钉集成」（2026-09-14 分批拆分）。整块搬移，未改 handler 逻辑。
 */

import { dingtalkBridgeManager } from '../lib/dingtalk-bridge-manager'
import { getDecryptedBotClientSecret, getDecryptedClientSecret, getDingTalkConfig, getDingTalkMultiBotConfig, removeDingTalkBot, saveDingTalkBotConfig, saveDingTalkConfig } from '../lib/dingtalk-config'
import { DINGTALK_IPC_CHANNELS } from '@guru/shared'
import type { DingTalkBridgeState, DingTalkConfig, DingTalkConfigInput, DingTalkTestResult } from '@guru/shared'
