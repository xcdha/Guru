...
import { FEISHU_IPC_CHANNELS, AGENT_IPC_CHANNELS } from '@guru/shared'
...
class FeishuBridge {
  ...
  stop(): void {
    ...
    // 重启后仍需用来给桌面 Session 镜像建群。完整重置请删除 ~/.guru/feishu-metadata-{botId}.json。
    this.botOpenId = null
   ...
   * source=session-mirror 的绑定回到同一个 Guru session。
   */
  ...
  private async createNewSession(
    ...
    if (!workspaceId) {
      await this.sendMessage(chatId, '请先在 Guru 设置中创建工作区。')
      return
    }
    ...
    if (!channelId) {
      await this.sendMessage(chatId, '请先在 Guru Agent 设置中选择渠道。')
      return
    }
  ...
  private async handleModelCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    if (channels.length === 0) {
      await this.sendMessage(
        chatId,
        '暂无可用渠道。请先在 Guru 设置中配置并启用渠道（需填入 API Key 且至少启用一个模型）。',
      )
      return
    }
    ...
    if (!targetBinding) {
      ...
      if (!targetBinding) {
        await this.sendMessage(chatId, '请先发送一条消息创建会话，或在 Guru 设置中选择 Agent 渠道。')
        return
      }
    }
  ...
  private async handleUserMessage(
    ...
    if (!session?.workspaceId) {
      ...
      await this.sendCardMessage(chatId, buildErrorCard('当前会话项目不可用，请在 Guru 中重新选择会话。'))
      return
    }
  ...
  private handleAgentPayload(sessionId: string, payload: AgentStreamPayload): void {
    ...
    if (payload.kind === 'guru_event' && payload.event.type === 'title_updated') {
