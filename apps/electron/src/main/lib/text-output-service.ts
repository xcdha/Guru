 ...
 * 文本输出服务
 *
 * 语音输入完成后优先写入 Guru 输入框，否则尝试写入当前光标位置。
 */
...
/** 在显示语音浮窗前记录目标是否为 Guru 主窗口。 */
export function captureVoiceDictationTarget(forceGuruInput?: boolean): boolean {
  const mainWindow = getMainWindow()
  targetWasGuruInput = forceGuruInput ?? BrowserWindow.getFocusedWindow() === mainWindow
  return targetWasGuruInput
}
...
function shouldWriteToGuruInput(settings: VoiceDictationSettings): boolean {
  return settings.outputMode === 'guru-input' ||
    (settings.outputMode === 'auto' && targetWasGuruInput)
}
...
function resolveVoiceDictationOutputContext(
  ...
  return {
    routeToGuruInput: shouldWriteToGuruInput(settings),
...
export function previewVoiceDictationText(
  ...
  if (!text || !outputContext?.routeToGuruInput) return
...
export async function commitVoiceDictationText(
  ...
  try {
    ...
    const shouldSendToGuruInput = hasActivePreview || outputContext.routeToGuruInput
    if (shouldSendToGuruInput) {
      ...
      if (delivered) {
        return { mode: 'guru-input', success: true, message: '已写入 Guru 输入框' }
      }

      clipboard.writeText(trimmed)
      return { mode: 'clipboard', success: true, message: 'Guru 输入框未确认接收，已复制到剪贴板' }
    }
