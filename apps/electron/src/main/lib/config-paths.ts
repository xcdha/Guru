...
/**
 * 解析 Guru 管理的默认 Markdown Vault 目录。
 */
...
/**
 * 获取 Guru 管理的默认 Markdown Vault 目录，并在首次使用时创建。
 *
 * @returns 正式版本 ~/.guru/vault/，开发模式 ~/.guru-dev/vault/
 */
 ...
 * 获取 Scratch Pad 文件路径。
 *
 * 保留原始旧版 scratch-pad.md，并按其内容指纹仅复制到 Guru 管理的默认 Vault 一次。
 * 已存在的 Vault 内容绝不覆盖；旧文件优先复制为草稿.md，重名时使用草稿 N.md。
 * 内容复制成功后以崩溃安全的状态文件记录指纹；重启或重复调用只返回 canonical Vault 路径。
 * 迁移失败时继续使用旧路径，以便下次安全重试。
 *
 * @returns 正式版本 ~/.guru/vault/scratch-pad.md，开发模式 ~/.guru-dev/vault/scratch-pad.md
 */
