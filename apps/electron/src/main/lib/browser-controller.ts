...
import type { BrowserExecutionSource, BrowserOperationStatus, BrowserSessionClosed, BrowserTabFocusChange, BrowserTraceAction, BrowserTraceItem, BrowserViewLayout, BrowserViewState, BrowserTabState } from '@guru/shared'
...
export class BrowserController {
  ...
  private createTab(browserSession: BrowserSessionRecord, isLocalPreview = false, claimAsAgent = false, popupOptions?: Pick<BrowserTabOptions, 'openedByPopup' | 'openerTabId' | 'popupInitialUrl' | 'viewOptions'>): BrowserTabRecord {
    ...
    view.webContents.setWindowOpenHandler(({ url }) => {
      ...
      return {
        action: 'allow',
        // Guru 将 window.open 呈现为同级浏览器 Tab；关闭来源页面不能级联关闭它。
        outlivesOpener: true,
