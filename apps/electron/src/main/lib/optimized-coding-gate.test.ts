...
import { createGuruSkillsOverride } from './adapters/pi-agent-adapter'
...
describe('createGuruSkillsOverride 开关联动', () => {
  ...
  test('开关关闭（默认）：gated skill 被过滤，普通 skill 保留', () => {
    const override = createGuruSkillsOverride([workspaceRoot], OPTIMIZED_CODING_GATED_SKILLS)
  ...
  test('开关开启：gated skill 放行', () => {
    const override = createGuruSkillsOverride([workspaceRoot], [])
    const result = override({ skills, diagnostics: [] } as never)
