 ...
 * - 消息路由到 MyYoda Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@myyoda/shared'
...
class DingTalkBridge {
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, '请先在 MyYoda 设置中选择 Agent 渠道。')
      return
    }
