 ...
 *
 * 这份名单是从 Guru 原样继承的"孤儿检测"黑名单模型：扫描 workspaceRoot 下的顶层目录，
 * 不在这份名单里、又不匹配任一活跃 session id / workspace slug 的，就当孤儿删除。
 * Guru 自身没有 projects/excalidraw，所以这份名单对它是完整的；Guru 之后独立新增的
...
const WORKSPACE_METADATA_DIRS = new Set([
  ...
  '.recovery-trash',
  'excalidraw',
  /** agent-workspace-manager.ts:getWorkspaceAutoMemoryDir — Guru Workspace Memory 根目录 */
  'memory',
