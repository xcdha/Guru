...
/** 官方站点（商业版 / 产品主页） */
export const GURU_OFFICIAL_URL = 'https://guru.cool'

/** 开源仓库完整地址 */
export const GURU_GITHUB_URL = 'https://github.com/guru-ai/Guru'

/** Commit trailer（标准 git trailer，不进入 GitHub co-author 列表） */
export const GURU_COMMIT_TRAILER = 'Made-with: Guru'
...
export const GURU_PR_ATTRIBUTION =
  `Made with [Guru](${GURU_OFFICIAL_URL}) · [GitHub](${GURU_GITHUB_URL})`
...
export function buildGitAttributionPromptSection(enabled: boolean): string {
  if (!enabled) {
    return `## Git / PR 标识
用户已关闭 Guru 标识；除非当次明确要求，不要添加任何 Guru 归因。`
  }

  return `## Git / PR 标识
当你代用户创建 commit 或 PR/MR 时，除非用户当次明确拒绝：
- commit 的 message 末尾追加唯一的 \`${GURU_COMMIT_TRAILER}\` trailer；不要使用 \`Co-Authored-By\`，不要改 author/committer。
- PR/MR 描述末尾追加唯一的 \`${GURU_PR_ATTRIBUTION}\`。`
}
