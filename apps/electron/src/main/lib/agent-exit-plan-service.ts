...
export interface ExitPlanModeCallbacks {
  /** 切换权限模式 */
  onPermissionModeChange: (mode: GuruPermissionMode) => void
}
...
export class AgentExitPlanService {
  ...
  handleExitPlanMode(
    ...
    if (!planDocument) {
      return Promise.resolve({
        ...
        message: '提交计划审批前必须先在当前会话计划目录（实际执行 cwd 的 .context/plan/）内写入 Markdown 计划文件（大小不超过 1 MB），再通过 planFile 传入其绝对路径',
      })
    }
   ...
   */
  respondToExitPlanMode(response: ExitPlanModeResponse): { sessionId: string; targetMode: GuruPermissionMode | null } | null {
  ...
  /**
   * 计划文档必须是当前会话计划目录内的真实 Markdown 常规文件。
